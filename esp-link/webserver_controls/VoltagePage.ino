...
void voltageLoop()
{
  uint16_t adc = analogRead(A2); // read ADC
  ...
  if( (sample_count % SAMPLE_COUNT) == 0 ) // max samples reached?
  {
    ...
    measured_voltage = ((5.0 / 1024) * voltage_avg) / (batteryDividerR2 / (batteryDividerR1 + batteryDividerR2)) ;
...
void voltageRefreshCb(char * url)
{
  ...
  String v = floatToString((float)measured_voltage);
  v += " V";
